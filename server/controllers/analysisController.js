import { analyzeSeoData } from "../geminiService.js";
...
// Analyze a URL
   ...
   try {
    ...
    try {
        validUrl = new URL(url.startsWith("http") ? url : `http://${url}`);
         ...
         return res.status(400).json({ success:false, message: "Invalis URL format"});
    }
 ...
 export const getAnalyses = async (req, res) => {
    ...
    const analyses = await  Analysis.find({ userid: req.userId}).sort({ createdAt: -1}).skip
    (skip).limit(limit).select("-issues -keywords");
